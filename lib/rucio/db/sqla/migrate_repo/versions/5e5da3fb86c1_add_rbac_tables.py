...
def upgrade():
    ...
    if context.get_context().dialect.name in ['oracle', 'mysql', 'postgresql']:
        ...
        create_foreign_key('ACCOUNT_ROLE_MAP_ACCOUNT_FK', 'account_role_map', 'accounts', ['account'], ['account'])
        create_foreign_key('ACCOUNT_ROLE_MAP_ROLE_FK', 'account_role_map', 'roles', ['role'], ['role'])
        ...
        create_check_constraint('ACCOUNT_ROLE_MAP_LOCKED_NN', 'account_role_map', 'locked is not null')
        ...
        create_table('role_permission_map',
                     ...
                     sa.Column('created_at', sa.DateTime, default=datetime.datetime.utcnow),
                     sa.Column('updated_at', sa.DateTime, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow))
        create_primary_key('ROLE_PERMISSION_MAP_PK', 'role_permission_map', ['role', 'scope', 'operation'])
        create_foreign_key('ROLE_PERMISSION_MAP_ROLE_FK', 'role_permission_map', 'roles', ['role'], ['role'])
        create_foreign_key('ROLE_PERMISSION_MAP_SCOPE_FK', 'role_permission_map', 'scopes', ['scope'], ['scope'])
        create_check_constraint('ROLE_PERMISSION_MAP_ROLE_NN', 'role_permission_map', 'role is not null')
        create_check_constraint('ROLE_PERMISSION_MAP_SCOPE_NN', 'role_permission_map', 'scope is not null')
